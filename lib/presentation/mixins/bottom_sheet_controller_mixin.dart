...
mixin BottomSheetControllerMixin<T extends StatefulWidget> on State<T>, TickerProviderStateMixin {
  ...
  double calculateSheetHeight(Size screenSize) {
    return bottomSheetController.value > 0
      ? collapsedSheetHeight + (bottomSheetController.value * (screenSize.height * expandedSheetHeight - collapsedSheetHeight))
      : 0;
  }
  ...
  void handleBottomSheetDrag(DragUpdateDetails details, Size screenSize) {
    ...
    final newValue = bottomSheetController.value - 
      (details.primaryDelta! / ((screenSize.height * expandedSheetHeight) - collapsedSheetHeight));
  ...
  void handleBottomSheetDragEnd(DragEndDetails details) {
    if (details.primaryVelocity! > 500 || bottomSheetController.value < 0.3) {
      collapseBottomSheet();
    } else if (details.primaryVelocity! < -500 || bottomSheetController.value > 0.7) {
      expandBottomSheet();
    } else {
